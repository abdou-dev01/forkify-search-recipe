...
class RecipeView extends View {
  ...
  addHandelUpdateServings(handler) {
    this._parentElement.addEventListener("click", function (event) {
      const btn = event.target.closest(".btn--update-servings");
      if (!btn) return;
      const newValue = +btn.dataset.updateTo;
      if (newValue > 0) handler(newValue);
    });
  }

  addHandelBookmarks(handler) {
    this._parentElement.addEventListener("click", function (event) {
      const btn = event.target.closest(".add-bookmark");
      if (!btn) return;
      handler();
    });
  }
